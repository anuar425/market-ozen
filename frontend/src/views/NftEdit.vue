<template>
  <el-dialog
    :title="modelValue._id ? 'Редактор NFT' : 'Создайте свой NFT'"
    :model-value="shown"
    @update:model-value="(v) => $emit('update:close', v)"
    width="420px"
  >
    <el-form ref="formEl" :model="form" label-width="150px" :rules="formRules">
      <el-form-item label="Изображение" prop="image">
        <image-uploader v-model="form.image" />
      </el-form-item>
      <el-form-item label="Символ" prop="sym">
        <el-input
          v-model="form.sym"
          :minlength="3"
          :maxlength="3"
          placeholder="Введите символ токена"
        ></el-input>
      </el-form-item>
      <el-form-item label="Имя" prop="name">
        <el-input v-model="form.name" placeholder="Введите имя токена"></el-input>
      </el-form-item>
      <el-form-item label="Описание">
        <el-input type="textarea" v-model="form.description" placeholder="Введите описание токена"></el-input>
      </el-form-item>
      <el-form-item label="Внешняя ссылка">
        <el-input type="textarea" v-model="form.externalUri" placeholder="Введите внешнюю ссылку"></el-input>
      </el-form-item>
    </el-form>
    <template #footer>
      <span class="dialog-footer">
        <el-button @click="$emit('update:shown', false)">Отмена</el-button>
        <el-button type="primary" @click="submit">{{ modelValue._id ? "Сохранить" : "Создать" }}</el-button>
      </span>
    </template>
  </el-dialog>
</template>

<script lang="ts">
const df = {
  image: "",
  sym: "",
  name: "",
  description: "",
  externalUri: "",
};

const formRules = {
  image: [{ required: true, message: "Требуется изображение" }],
  sym: [
    { required: true, message: "Требуется символ токена" },
    {
      min: 3,
      max: 3,
      message: "Символ должен состоять из 3 символов",
    },
  ],
  name: [
    { required: true, message: "Требуется имя токена" },
    {
      min: 2,
      max: 26,
      message: "Имя должно быть от 2 до 26 символов",
    },
  ],
};
import { defineComponent, PropType, ref, toRef, watch } from "vue";
import { ElFormItemContext } from "element-plus/lib/el-form";
import { useStore } from "vuex";
import { NFT } from "@/types";
import imageUploader from "@/components/image-uploader.vue";
export default defineComponent({
  components: { imageUploader },
  name: "NftEdit",
  emits: ["update:modelValue", "update:shown"],
  props: {
    modelValue: {
      type: Object as PropType<NFT>,
      default: () => df,
    },
    shown: Boolean,
  },
  setup(props, { emit }) {
    const formEl = ref<ElFormItemContext>();
    const form = ref(df);
    const store = useStore();
    watch(toRef(props, "shown"), (nw) => {
      if (nw) {
        console.log("x");
        form.value = props.modelValue;
      }
    });
    function submit() {
      // form validate
      formEl.value?.validate((valid?: boolean) => {
        if (!valid) return false;
        // validated
        store.dispatch("nft/save", form.value);
        emit("update:shown", false);
        //form.value = df;
      });
    }
    return { submit, form, formEl, formRules };
  },
});
</script>
/script>
